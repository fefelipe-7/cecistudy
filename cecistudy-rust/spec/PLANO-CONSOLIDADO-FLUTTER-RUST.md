...
## ⚠️ Status de implementação (atualizado 2026-09-12)
...
- **Fase 1 (núcleo Rust): 21/23 ✅** (corrigido por auditoria 2026-09-12 — a numeração anterior "20/23"
  superestimava o real; ver breakdown). Workspace + `cecistudy-common/domain/data/sync/content/app`
  implementados nos commits `5ee35fb`→`bf9790a` (gate: clippy -D warnings ✓, fmt ✓, **162 testes** ✓).
  Módulos de domínio `calendar/knowledge/marketing/projects/internship` **portados** (R2, spec-first —
  breakdown + `05-modulos-greenfield.md` + `schema.sql`). Pendentes: validação de paridade full (1.22)
  e `cecistudy-ffi` (1.21) — além do rework R3/R4 (superfície do `cecistudy-content` e remoção do
  `data_json` exposto).
- **Fase 2+ (Flutter): 🔲 não iniciada** — exige Fase 1 fechada e FFI (1.21) concluído.
- **Próximo passo real:** fechar a paridade full (1.22) → remover o `data_json` exposto no
  `cecistudy-data` (R4) → `cecistudy-ffi` (1.21) para destravar a Fase 2.
  Detalhe por passo: seção "Próximos passos" do breakdown.

#!/usr/bin/env node
/**
 * Gera `src/data/psicoterapiaBase.ts` a partir das entregas de
 * `library/cecistudy_base_psicoterapias/entregas/`.
 *
 ...
 * Saída: src/data/psicoterapiaBase.ts (gerado, commitado).
 */
...
function main() {
  ...
  const ts = `// GENERATED BY scripts/build-psicoterapia.mjs — NÃO EDITAR MANUALMENTE.
// Fonte: library/cecistudy_base_psicoterapias/entregas/*.md (97 abordagens, 10 famílias).
import type { PsychologyApproach, PsicoterapiaFamily } from '../types';
...
`;
  ...
  console.log(`✅ gerado ${path.relative(ROOT, OUT_FILE)} — ${total} abordagens`);

...
const SUB_TABS = ['disciplinas', 'aulas', 'avaliacoes', 'calendario'] as const;
...
export const FaculdadeView: React.FC = () => {
  ...
  if (focusedCourse) {
    return (
      <SwipeBack onClose={closeCourseDetail}>
        <CourseDetailView
  ...
  return (
    <div className="max-w-md sm:max-w-xl mx-auto space-y-6 pb-1">
      ...
      <SwipeTabPager
        tabs={SUB_TABS}
        activeIndex={Math.max(0, SUB_TABS.indexOf(subTab as (typeof SUB_TABS)[number]))}
        mode="nested"
        onChange={(i) => setSubTab(SUB_TABS[i] as SubTabFaculdade)}
        onEdgeOverscroll={(dir) =>
          handleNavigate(TAB_ORDER[TAB_ORDER.indexOf('faculdade') + dir])
        }
      >
        {(id) => (
          <>

...
| **Técnicas do templo** | 135 técnicas canônicas são somente-leitura; a entidade pessoal `Technique` (`handleAddTechnique`) existe mas nenhuma ponte acervo→pessoal foi feita (só conceitos e autores). | Replicar o padrão `adoptAcervo*` para técnicas quando houver tela que selecione técnica pessoal. |

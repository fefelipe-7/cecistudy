...
export function maxStamp(index: SyncIndex | undefined): number {
  if (!index) return 0;
  let max = 0;
  for (const v of Object.values(index.stamps ?? {})) max = Math.max(max, v);
  for (const rec of Object.values(index.records ?? {})) {
    for (const v of Object.values(rec)) max = Math.max(max, v);
  }
  for (const rec of Object.values(index.tombstones ?? {})) {
    for (const v of Object.values(rec)) max = Math.max(max, v);
  }
  return max;
}
